...
We have used parallel cooridinate plot to interactively understand the relationship between multiple weather variables and ridership together.
...
Our analysis indicate that there are many more riders in the summer than in winter. It is not surprising as commuters tend to prefer safer mode of transportation when it’s freezing cold.  This got us to dig deep into the impact of weather on bike and taxi ridership in NYC.

The relationship between bike ridership and weather could be described as non-linear.  The ridership is less during the colder days as expected. Our initial hypothesis was that the warmer it gets in NYC, the more open people are to use Citibike. However, it is to be noted that people change their behavior not only at low temperatures but also at high temperatures. We saw a similar pattern in average duration where trip duration increases during warmer days and pattern reverse slightly in the last decile of temperatures.

We also noticed that the ridership has decreased as the precipitation increased. However the drop in ridership is significantly higher when the precipitation is >1, suggesting that NYC riders are resilient to the precipitation to an extent.
